...
jQuery.fn.hubSlider = function(config) {
	...
	try {
		...
		var off;
		var offx
		settings.button.next.on("touchstart",function(e) {
			
			e.preventDefault()
			off = e.originalEvent.targetTouches[0].pageX
			settings.button.next.on("touchmove",function(e){
				offx=off-e.originalEvent.targetTouches[0].pageX
			})
		})
		settings.button.next.on("touchend",function(e) {
			...
			if(offx >100) {
				...
				console.log("x>0")
				movement('next');
				$(this).addClass("unactive").delay(200).queue(function(){
					$(this).removeClass("unactive").dequeue()
				})
			...
			if (offx <-100){
				...
				// $(this).closest("ul").find("li").attr("").addClass("active").delay(200).queue(function(){
				// 		$(this).closest("ul").find("li:last").removeClass("active").dequeue()
				// })
				$("html .body").unbind("touchstart");
				$("html .body").unbind("touchmove");
				$("html .body").unbind("touchend")
				console.log("x<0")
				$(they).find("li").each(function(i,v){
					if($(v).attr("data-key")==$(they).find("li").length-1){
						$(v).addClass("active").delay(200).queue(function(){
						movement('prev')
						$(v).removeClass("active").dequeue()
				})
